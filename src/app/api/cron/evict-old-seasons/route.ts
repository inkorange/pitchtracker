...
// ONLY the current season by default; older data is lazily re-fetched
// from Savant on demand via ensurePitcherSeasonCache when a user
...
// many EXTRA seasons of history we keep (default 0 = current season
// only). Set to 1 to also keep the previous season, 2 for two priors.
// Tightened from the old default of 1 because pitch_game_pitches
// was approaching the DB storage quota (500MB+ at 2-season retention).
...
export async function GET(request: Request) {
  ...
  const keepYears = Number(process.env.PITCH_DATA_KEEP_YEARS ?? "0");
